...
- **Imports**: Use `ui` package for ALL Chakra UI components (never import from @chakra-ui directly), `workspace:*` for internal deps, `catalog:` for shared versions (react, typescript, zod, vitest, @testing-library/\*, @types/node, @types/aws-lambda). Import from `sst-constructs` for shared SST constructs (SsrSite, NitroSite).
